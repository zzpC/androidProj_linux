...
Java_com_example_wynews_VideoFragment_plays(JNIEnv *env, jobject instance, jstring videoPath_,
                                            jobject surface) {
    const char * input_char = (*env)->GetStringUTFChars(env,videoPath_,NULL);
    ...
    LOGE("视频的"
                 "宽高：%d,%d",pCodecCtx->width,pCodecCtx->height);
...
//                usleep(1);
    ...
    (*env)->ReleaseStringUTFChars(env,videoPath_,input_char);
