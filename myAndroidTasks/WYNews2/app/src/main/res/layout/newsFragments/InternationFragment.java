...
public class InternationFragment extends BaseFragment {
    //定制化的新闻列表
    private ListViewLoadMore mInternationFragment;
    //网络抓取来的数据
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SHOW_NEWS_TITLE:
                    mInternationFragment.setAdapter(mListviewAdapter);
                    break;
            }
        }
    };
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mInternationFragment = (ListViewLoadMore) view.findViewById(R.id.social_list_view);

        mListviewAdapter = new NewsAdapter(getContext(), R.layout.listitem_home, mRawList);


        if (mInternationFragment.getCount() == 0) {
            initNews();

        }

        mInternationFragment.setListener(new ListViewLoadMore.OnListener() {
            @Override
    ...
    private void loadmoreExecute(){
        ...
        mInternationFragment.setLoadCompleted();
    }
