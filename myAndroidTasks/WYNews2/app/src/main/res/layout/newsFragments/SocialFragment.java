...
public class SocialFragment extends BaseFragment {
    //定制化的新闻列表
    private ListViewLoadMore mSocialListView;
    //网络抓取来的数据
    private List<News> mRawList = new ArrayList<>();
    //网络原生数据的适配
    private NewsAdapter mListviewAdapter;
    //新闻标题发送完成
    private static final int SHOW_NEWS_TITLE = 20;

    private  Handler mhandler = new Handler() {
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SHOW_NEWS_TITLE:
                    mSocialListView.setAdapter(mListviewAdapter);
                    break;
            }
        }
    };
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mSocialListView = view.findViewById(R.id.social_list_view);

        mListviewAdapter = new NewsAdapter(getContext(), R.layout.listitem_home, mRawList);


        if (mSocialListView.getCount() == 0) {
            initNews();

        }

        mSocialListView.setListener(new ListViewLoadMore.OnListener() {
            @Override
    ...
    private void loadmoreExecute(){
        ...
        mSocialListView.setLoadCompleted();
    }
